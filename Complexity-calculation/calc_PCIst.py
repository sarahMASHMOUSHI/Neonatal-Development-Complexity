...
def calc_PCIst(signal_evk, times, full_return=False, **par):
    ...
    **pars : dictionary
    ...
    # Preprocess signal based on specified baseline and response windows
    ...
    signal_svd, var_exp, eigenvalues, snrs, pre_signal_svd, Vk, Nc = dimensionality_reduction(signal_evk, times, **par) 
    
    # Quantify state transitions from reduced data
    STQ = state_transition_quantification(signal_svd, times, **par)

    # Calculate the Perturbational Complexity Index (PCI) by summing up state transitions
    PCI = np.sum(STQ['dNST'])                     

    if full_return:                              
        return {'PCI': PCI, **STQ, 'signal_evk': signal_evk, 'times': times, 'signal_svd': signal_svd,
                'eigenvalues': eigenvalues, 'var_exp': var_exp, 'snrs': snrs, 'pre_signal_svd': pre_signal_svd, 'Vk': Vk, 'Nc': Nc}
    return PCI 
    

## DIMENSIONALITY REDUCTION
def dimensionality_reduction(signal, times, response_window, max_var=99, min_snr=1,
    ...
    # Get SVD results within the response window
    ...
    pre_signal_svd = apply_svd(signal, Vk) 

    max_dim = calc_maxdim(eigenvalues, max_var)

    pre_signal_svd = pre_signal_svd[:max_dim, :]

    snrs = calc_snr(pre_signal_svd, times, kwargs['baseline_window'], response_window)
    signal_svd = pre_signal_svd[snrs > min_snr, :]
    snrs = snrs[snrs > min_snr]

    Nc = signal_svd.shape[0]

    return signal_svd, var_exp[:Nc], eigenvalues, snrs, pre_signal_svd, Vk, Nc 
...
def state_transition_quantification(signal, times, k, baseline_window, response_window, embed=False,
    ...
    for i in range(n_dims):
        ...
        # The minimum threshold is defined as the median of the baseline’s distance matrix (D_base), 
        # which represents the typical distance among points in the baseline period.
        min_thr = np.median(D_base[i, :, :].flatten()) 

        # The maximum threshold is calculated as the maximum value in the response distance matrix (D_resp) scaled by max_thr_p (a multiplier, often set to 1.0). 
        # This captures the highest level of transition seen during the response.
    ...
    # The array max_thresholds thus contains the specific threshold for each component that maximizes the NST difference, making it "optimal" per component
...
## PREPROCESS
def preprocess_signal(signal_evk, times, time_window, baseline_corr=True, resample=None,
                      avgref=False, **kwargs):
    assert signal_evk.shape[1] == len(times), 'Signal and Time arrays must be of the same size.'
    if avgref:
        signal_evk = avgreference(signal_evk)
    if baseline_corr:
        signal_evk = baseline_correct(signal_evk, times, delta=2) # baseline window = first 2 seconds of the signal
...
def baseline_correct(Y, times, delta=0):
    ...
    assert onset_ix > 0, "Baseline period is invalid (empty slice)." 
